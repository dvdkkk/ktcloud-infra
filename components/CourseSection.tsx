...
const CURRICULUM_DATA = [
  ...
  { id: "02", title: "클라우드 서비스를 위한 네트워크 기본", desc: "클라우드 서비스 이용과 리소스간 통신을 위한 시작과 끝", subjects: ["데이터 센터 네트워크 기술", "접근 제어와 트래픽 관리", "라우팅과 스위칭", "클라우드 데이터 센터 운영을 위한 BGP, MPLS"] },
  ...
  { id: "06", title: "오픈소스 쿠버네티스 아키텍처 설계 및 구현", desc: "클라우드 네이티브기반의 개발과 운영을 위한 모든 것", subjects: ["Pod 기반의 애플리케이션 배포", "서비스, 인그레스를 이용한 애플리케이션 접속", "Pod 자원관리 및 모니터링", "k8s 기반의 CI.CD", "고가용성 기반의 k8s 클러스터(멀티 master, etcd 백업/복구)"] },
  ...
  { id: "11", title: "고가용성 중심의 AWS 아키텍처 설계", desc: "심층적인 AWS 서비스를 이용하기 위한 방법", subjects: ["AWS 기반의 IaC", "서버리스 아키텍처", "멀티 티어 아키텍처 설계", "멀티 리전, 멀티 존 구성", "ASG/keda + karpenter 기반의 오토 스케일링"] },
  ...
  { id: "14", title: "서비스메시 기반의 MSA 설계 및 관리", desc: "통신을 제어하고 모니터링하는 서비스 메시 솔루션", subjects: ["서비스 메시의 이해", "이스티오 게이트웨이와 프락시", "네트워크 제어하기"] },
  { id: "15", title: "오픈스택 기반 프라이빗 클라우드 구현", desc: "클라우드 환경을 직접 구축하는 오픈소스 컨테이너 플랫폼", subjects: ["컨테이너 기반의 오픈스택 배포(kolla-ansible)", "네트워크/컨트롤/스토리지/컴퓨트 서비스", "오픈스택환경에서의 쿠버네티스", "로드밸런서(octavia), kubernetes 서비스 환경 구성", "(토이프로젝트) 오픈스택 환경에서의 웹서비스 환경 구현 (octavia, nova, trove, cinder)"] },
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {CURRICULUM_DATA.map((item) => (
            <div key={item.id} className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl">
              <span className="text-red-500 font-black text-2xl mb-2 block">{item.id}.</span>
              <h3 className="text-xl font-bold mb-2">{item.title}</h3>
              <p className="text-zinc-400 text-sm mb-4">{item.desc}</p>
              <ul className="space-y-2 text-zinc-300 text-sm">
                {item.subjects.map((sub, i) => <li key={i}>• {sub}</li>)}
              </ul>
            </div>
          ))}
        </div>
        ...
        <div className="bg-zinc-900 p-8 rounded-3xl">
            <h3 className="text-2xl font-bold mb-8 text-center">프로젝트 과정</h3>
            <div className="grid md:grid-cols-3 gap-8">
                {PROJECTS_DATA.map((p, i) => (
                    <div key={i} className="bg-black p-6 rounded-2xl border border-zinc-800">
                        <h4 className="text-lg font-bold mb-4 text-red-500">{p.title}</h4>
                        <ul className="space-y-2 text-zinc-300 text-sm">
                            {p.subjects.map((sub, j) => <li key={j}>- {sub}</li>)}
                        </ul>
                    </div>
                ))}
            </div>
            <p className="text-zinc-500 text-xs mt-8 text-center">※ 토이프로젝트는 강의 시간중 오후 시간을 할애하여 2~3일 만에 완성함.</p>
        </div>
      </div>
    </section>
  );
